...
#IMPORT FILES###################################################################
load("data-raw/temp/simmeta.RData")

ppt_dist <- setDT(read.csv("data-raw/source/ppt_Dist.csv", header = TRUE, fileEncoding="UTF-8-BOM"))#Self defined variable distribution
ppt_Mdist <- setDT(read.csv("data-raw/source/ppt_MissDist.csv", header = TRUE, fileEncoding="UTF-8-BOM")) #defines the parameters of missing
...
#SAVE DATA######################################################################
save(simmeta, ppt_all, file = "data-raw/temp/simdatatemp.RData")
