          ...
          div(
            ...
            wellPanel( #### Data Availability  #####################################
              ...
              fluidRow(
                ...
                column(3, my_yn_inputs(NS(id, "cat16"), "OtherF")) #!UPDATES!#
              )
            )
          )
            ...
            wellPanel( #!UPDATES!#
              ...
              fluidRow(
                column(
                  4,
                  wellPanel(
                    sliderInput(NS(id, "mmse"), "MMSE", min = 0, max = 30, value = c(0, 30)),
                    checkboxInput(NS(id, "mmsemiss"), "Include Missing", value = TRUE))))
            )
          )
...
filters_mod_server <- function(id, df1, df2, df3) {
  moduleServer(id, function(input, output, session) {
    ...
    output$filter_sel <- DT::renderDataTable({
      ...
      # Default values
      ppt_inclumiss_range <- toupper(rep(TRUE, 8)) #!UPDATES! (change from 7 to 8)

      
      df <- data.frame(VARNAME = 1:60) #!UPDATES! (change from 57 to 60)
...
## Module App ##################################################################
filter_mod_app <- function(df1, df2, df3) {
  ui <- fluidPage(
